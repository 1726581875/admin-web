...
/*        let key = file.name + file.size + file.type;
...
/*        let shardSize = _this.shardSize;
        ...
        console.log("文件上传开始");
        let formData = new window.FormData();
        let fileShard = _this.getFileShard(3, 10 * 1024 * 1024);


        formData.append("fileShard",fileShard);
        formData.append("fileKey","aaaa");
        formData.append("shard",3);
          _this.$ajax.post("http://localhost:9000/fileSystem/file/upload/big", formData).then((response) => {
            console.log("上传文件成功：");
          });
...
/*        let param = {
      ...
      upload (param) {
        ...
        fileReader.onload = function (e) {
          ...
          param.shard = base64;
          _this.$ajax.post("http://localhost:9000/fileSystem/file/upload/big", param).then((response) => {
            let resp = response.data;
