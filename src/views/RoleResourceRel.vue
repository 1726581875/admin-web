<template>
    <div>
        ...
        <div class="container">
            <div class="handle-box">
                ...
                <el-select v-model="selectValue" placeholder="查询条件" class="handle-select mr10">
                        ...
                        <el-option key="1" label="角色ID" value="roleId"></el-option>
                        <el-option key="2" label="资源ID" value="resourceId"></el-option>
                </el-select>
                <!--  查找输入框 -->
                <el-input
                        v-model="inputVale"
        ...
        <el-dialog :title="dialogTitle" :visible.sync="editVisible" width="40%">
            <el-form ref="roleResourceRel" :model="roleResourceRel" label-width="70px">
                        <el-form-item label="角色ID">
                            <el-input v-model="roleResourceRel.roleId"></el-input>
                        </el-form-item>
                        <el-form-item label="资源ID">
                            <el-input v-model="roleResourceRel.resourceId"></el-input>
                        </el-form-item>
                        <el-form-item label="创建时间">
                            <el-input v-model="roleResourceRel.createTime"></el-input>
                        </el-form-item>
                        <el-form-item label="修改时间">
                            <el-input v-model="roleResourceRel.updateTime"></el-input>
                        </el-form-item>
            </el-form>
...
<script>
  export default {
    ...
    methods: {
       ...
       */
      list() {
        this.$axios.get('http://localhost:9001/admin/roleResourceRels/list', {
          params: this.queryParam
        }).then(res => {
          let result = res.data;
          if (result.success) {
            this.roleResourceRelList = result.data.content;
       ...
       */
      deleteById(id){
        this.$axios.delete("http://localhost:9001/admin/roleResourceRels/" + id)
          .then(res => {
      ...
      deleteSelected() {
        ...
        // 1、获取到要删除的roleResourceRel id数组
        const roleResourceRelIdList = [];
        // 名字 list
        let delRoleResourceRelNames = [];
        this.multipleSelection.forEach((roleResourceRel, index) => {
          // 拿到要删除的roleResourceRelId
          roleResourceRelIdList.push(roleResourceRel.id);
          // 取前五个名字
          if (index <= 5) {
            delRoleResourceRelNames.push(roleResourceRel.name);
          }
        });
        let delCount = roleResourceRelIdList.length;
        // 批量删除的名字拼接 String
        let delMsgStr = delCount <= 5 ? delRoleResourceRelNames.toString() : delRoleResourceRelNames.toString() + '...';

        //2、弹出框，提示信息,二次确认
        this.$confirm(`确定要批量删除 [<span style="color: red">${delMsgStr}</span>] 一共
         <span style="color: red">${roleResourceRelIdList.length}</span> 列吗?`, '提示', {
          ...
          //3、发送批量删除请求
          this.$axios.post("http://localhost:9001/admin/roleResourceRels/batch/delete", roleResourceRelIdList)
            .then(res => {
      ...
      handleEdit(index, row) {
        ...
        let newRoleResourceRel = JSON.parse(JSON.stringify(row));
        this.roleResourceRel = newRoleResourceRel;
        // 展示编辑框
      ...
      save() {
        ...
        //3、发请求
        this.$axios.post('http://localhost:9001/admin/roleResourceRels/roleResourceRel', this.roleResourceRel)
          .then(res => {
            if (res.data.success) {
