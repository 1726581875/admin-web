<template>
    <div>
        ...
        <div class="container">
            <div class="handle-box">
                ...
                <el-select v-model="selectValue" placeholder="查询条件" class="handle-select mr10">
                        ...
                        <el-option key="1" label="文件名" value="name"></el-option>
                        <el-option key="2" label="视频相对路径" value="filePath"></el-option>
                        <el-option key="3" label="大小" value="fileSize"></el-option>
                        <el-option key="4" label="文件后缀" value="fileSuffix"></el-option>
                        <el-option key="5" label="文件唯一标识" value="fileKey"></el-option>
                        <el-option key="6" label="分片下标" value="shardIndex"></el-option>
                        <el-option key="7" label="总共分片数" value="shardCount"></el-option>
                        <el-option key="8" label="分片大小" value="shardSize"></el-option>
                </el-select>
                <!--  查找输入框 -->
                <el-input
                        v-model="inputVale"
        ...
        <el-dialog :title="dialogTitle" :visible.sync="editVisible" width="40%">
            <el-form ref="moocFile" :model="moocFile" label-width="70px">
                        <el-form-item label="文件名">
                            <el-input v-model="moocFile.name"></el-input>
                        </el-form-item>
                        <el-form-item label="视频相对路径">
                            <el-input v-model="moocFile.filePath"></el-input>
                        </el-form-item>
                        <el-form-item label="大小">
                            <el-input v-model="moocFile.fileSize"></el-input>
                        </el-form-item>
                        ...
                        <el-form-item label="分片下标">
                            <el-input v-model="moocFile.shardIndex"></el-input>
                        </el-form-item>
                        <el-form-item label="总共分片数">
                            <el-input v-model="moocFile.shardCount"></el-input>
                        </el-form-item>
                        <el-form-item label="分片大小">
                            <el-input v-model="moocFile.shardSize"></el-input>
                        </el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      deleteSelected() {
        ...
        // 1、获取到要删除的moocFile id数组
        const moocFileIdList = [];
        // 名字 list
        let delMoocFileNames = [];
        this.multipleSelection.forEach((moocFile, index) => {
          // 拿到要删除的moocFileId
          moocFileIdList.push(moocFile.id);
          // 取前五个名字
          if (index <= 5) {
            delMoocFileNames.push(moocFile.name);
          }
        });
        let delCount = moocFileIdList.length;
        // 批量删除的名字拼接 String
        let delMsgStr = delCount <= 5 ? delMoocFileNames.toString() : delMoocFileNames.toString() + '...';

        //2、弹出框，提示信息,二次确认
        this.$confirm(`确定要批量删除 [<span style="color: red">${delMsgStr}</span>] 一共
         <span style="color: red">${moocFileIdList.length}</span> 列吗?`, '提示', {
      ...
      handleEdit(index, row) {
        ...
        let newMoocFile = JSON.parse(JSON.stringify(row));
        this.moocFile = newMoocFile;
        // 展示编辑框
