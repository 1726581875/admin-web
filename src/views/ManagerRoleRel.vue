<template>
    <div>
        ...
        <div class="container">
            <div class="handle-box">
                ...
                <el-select v-model="selectValue" placeholder="查询条件" class="handle-select mr10">
                        ...
                        <el-option key="1" label="管理员ID" value="managerId"></el-option>
                        <el-option key="2" label="角色ID" value="roleId"></el-option>
                </el-select>
                <!--  查找输入框 -->
                <el-input
                        v-model="inputVale"
        ...
        <el-dialog :title="dialogTitle" :visible.sync="editVisible" width="40%">
            <el-form ref="managerRoleRel" :model="managerRoleRel" label-width="70px">
                        <el-form-item label="管理员ID">
                            <el-input v-model="managerRoleRel.managerId"></el-input>
                        </el-form-item>
                        <el-form-item label="角色ID">
                            <el-input v-model="managerRoleRel.roleId"></el-input>
                        </el-form-item>
                        <el-form-item label="创建时间">
                            <el-input v-model="managerRoleRel.createTime"></el-input>
                        </el-form-item>
                        <el-form-item label="修改时间">
                            <el-input v-model="managerRoleRel.updateTime"></el-input>
                        </el-form-item>
            </el-form>
...
<script>
  export default {
    ...
    methods: {
       ...
       */
      list() {
        this.$axios.get('http://localhost:9001/admin/managerRoleRels/list', {
          params: this.queryParam
        }).then(res => {
          let result = res.data;
          if (result.success) {
            this.managerRoleRelList = result.data.content;
       ...
       */
      deleteById(id){
        this.$axios.delete("http://localhost:9001/admin/managerRoleRels/" + id)
          .then(res => {
      ...
      deleteSelected() {
        ...
        // 1、获取到要删除的managerRoleRel id数组
        const managerRoleRelIdList = [];
        // 名字 list
        let delManagerRoleRelNames = [];
        this.multipleSelection.forEach((managerRoleRel, index) => {
          // 拿到要删除的managerRoleRelId
          managerRoleRelIdList.push(managerRoleRel.id);
          // 取前五个名字
          if (index <= 5) {
            delManagerRoleRelNames.push(managerRoleRel.name);
          }
        });
        let delCount = managerRoleRelIdList.length;
        // 批量删除的名字拼接 String
        let delMsgStr = delCount <= 5 ? delManagerRoleRelNames.toString() : delManagerRoleRelNames.toString() + '...';

        //2、弹出框，提示信息,二次确认
        this.$confirm(`确定要批量删除 [<span style="color: red">${delMsgStr}</span>] 一共
         <span style="color: red">${managerRoleRelIdList.length}</span> 列吗?`, '提示', {
          ...
          //3、发送批量删除请求
          this.$axios.post("http://localhost:9001/admin/managerRoleRels/batch/delete", managerRoleRelIdList)
            .then(res => {
      ...
      handleEdit(index, row) {
        ...
        let newManagerRoleRel = JSON.parse(JSON.stringify(row));
        this.managerRoleRel = newManagerRoleRel;
        // 展示编辑框
      ...
      save() {
        ...
        //3、发请求
        this.$axios.post('http://localhost:9001/admin/managerRoleRels/managerRoleRel', this.managerRoleRel)
          .then(res => {
            if (res.data.success) {
