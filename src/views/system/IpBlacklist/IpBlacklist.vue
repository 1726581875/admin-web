<template>
    <div>
        <div class="container">
            ...
            >
                <el-table-column type="selection" width="55" align="center"/>
                        ...
                        <el-table-column prop="ip" label="禁用IP"/>
        ...
        <el-dialog :title="dialogTitle" :visible.sync="editVisible" width="40%">
            <el-form ref="ipBlacklist" :model="ipBlacklist" label-width="70px">
                        <el-form-item label="名称">
                            <el-input v-model="ipBlacklist.name"></el-input>
                        </el-form-item>
                        <el-form-item label="禁用ip">
                            <el-input v-model="ipBlacklist.ip"></el-input>
                        </el-form-item>
            </el-form>
...
<script>
  export default {
    ...
    methods: {
       ...
       */
      list() {
        this.$axios.get('http://localhost:9001/admin/ipBlacklists/list', {
          params: this.queryParam
        }).then(res => {
          let result = res.data;
          if (result.success) {
            this.ipBlacklistList = result.data.content;
      ...
      handleDelete(index, id) {
        // 弹框，二次确认删除
        this.$confirm('确定要把该IP从黑名单移除吗？', '提示', {
          type: 'warning'
        }).then(() => { // 点击确认删除
          this.deleteById(id);
        }).catch(() => {// 点击取消
       ...
       */
      deleteById(id){
        this.$axios.delete("http://localhost:9001/admin/ipBlacklists/" + id)
          .then(res => {
      ...
      deleteSelected() {
        ...
        // 1、获取到要删除的ipBlacklist id数组
        const ipBlacklistIdList = [];
        // 名字 list
        let delIpBlacklistNames = [];
        this.multipleSelection.forEach((ipBlacklist, index) => {
          // 拿到要删除的ipBlacklistId
          ipBlacklistIdList.push(ipBlacklist.id);
          // 取前五个名字
          if (index <= 5) {
            delIpBlacklistNames.push(ipBlacklist.name);
          }
        });
        let delCount = ipBlacklistIdList.length;
        // 批量删除的名字拼接 String
        let delMsgStr = delCount <= 5 ? delIpBlacklistNames.toString() : delIpBlacklistNames.toString() + '...';

        //2、弹出框，提示信息,二次确认
        this.$confirm(`确定要批量删除 [<span style="color: red">${delMsgStr}</span>] 一共
         <span style="color: red">${ipBlacklistIdList.length}</span> 列吗?`, '提示', {
          ...
          //3、发送批量删除请求
          this.$axios.post("http://localhost:9001/admin/ipBlacklists/batch/delete", ipBlacklistIdList)
            .then(res => {
        ...
        handleEdit: function (index, row) {
            ...
            let newIpBlacklist = JSON.parse(JSON.stringify(row));
            this.ipBlacklist = newIpBlacklist;
            // 展示编辑框
      ...
      save() {
          ...
          //3、发请求
          this.$axios.post('http://localhost:9001/admin/ipBlacklists/ipBlacklist', this.ipBlacklist)
                  .then(res => {
                      if (res.data.success) {
