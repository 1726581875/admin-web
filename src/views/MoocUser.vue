        ...
        <div class="container">
            <div class="handle-box">
                ...
                <el-select v-model="selectValue" placeholder="查询条件" class="handle-select mr10">
                        ...
                        <el-option key="1" label="用户头像" value="userIamge"></el-option>
                        <el-option key="2" label="用户昵称" value="name"></el-option>
                        <el-option key="3" label="登录账号" value="account"></el-option>
                        <el-option key="4" label="登录密码" value="password"></el-option>
                        <el-option key="5" label="类型，教师/普通用户" value="userType"></el-option>
                        <el-option key="6" label="用户状态" value="status"></el-option>
                </el-select>
                <!--  查找输入框 -->
                <el-input
                        v-model="inputVale"
            ...
            >
                <el-table-column type="selection" width="55" align="center"/>
                        ...
                        <el-table-column prop="userType" label="类型，教师/普通用户"/>
...
<script>
  export default {
    ...
    methods: {
       ...
       */
      deleteById(id){
        this.$axios.delete("http://localhost:9001/admin/moocUsers/" + id)
          .then(res => {
      ...
      deleteSelected() {
        ...
        // 1、获取到要删除的moocUser id数组
        const moocUserIdList = [];
        // 名字 list
        let delMoocUserNames = [];
        this.multipleSelection.forEach((moocUser, index) => {
          // 拿到要删除的moocUserId
          moocUserIdList.push(moocUser.id);
          // 取前五个名字
          if (index <= 5) {
            delMoocUserNames.push(moocUser.name);
          }
        });
        let delCount = moocUserIdList.length;
        // 批量删除的名字拼接 String
        let delMsgStr = delCount <= 5 ? delMoocUserNames.toString() : delMoocUserNames.toString() + '...';

        //2、弹出框，提示信息,二次确认
        this.$confirm(`确定要批量删除 [<span style="color: red">${delMsgStr}</span>] 一共
         <span style="color: red">${moocUserIdList.length}</span> 列吗?`, '提示', {
          ...
          //3、发送批量删除请求
          this.$axios.post("http://localhost:9001/admin/moocUsers/batch/delete", moocUserIdList)
            .then(res => {
